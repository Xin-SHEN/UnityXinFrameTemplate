...
public class MP_ScreenGrid : MonoBehaviour
{
    ...
    class EDGE
	{
        ...
        public MP_Edge mp_edge = null;
        public float preloadGamma = 0;
	};
	EDGE [] edge_sides = new EDGE [4] ;

    EDGE focusedEdge = null; // current modified edge
    ...
    /// <returns></returns>
    Transform made_a_edge(string name, int edge_id)
	{
		...
		if(EdgeTrm==null)
		{
            ...
            //float gamma = PlayerPrefs.GetFloat(getGammaKeyStr(edge_id), 0.31f);
            //update_gamma(gamma, false);
            float temp_gamma = PlayerPrefs.GetFloat(getGammaKeyStr(edge_id), 0.31f);
            go.GetComponent<MP_Edge>().UpdateGamma(temp_gamma,false);
            //update_gamma(go.GetComponent<MP_Edge>().gamma, false);
        }
		return EdgeTrm;
	}
    ...
    void made_edge_up()
	{
		...
		for (int i = 0; i <  grid_array.pCol.Count; i++) { 
			...
			for (int j = grid_array.pCol [i].pRow.Count-blend_count; j <  grid_array.pCol [i].pRow.Count; j++) {				
				grid_array.pCol [i].pRow [j].index = index;
				index++;
		...
		Transform EdgeTrm =made_a_edge("Edge_up",2);
    ...
    void made_edge_down()
	{
		...
		Transform EdgeTrm =made_a_edge("Edge_down",3);
    ...
    void made_edge_right()
	{
		...
		Transform EdgeTrm =made_a_edge("Edge_right",1);
    ...
    void made_edge_left()
	{
		...
		Transform EdgeTrm =made_a_edge("Edge_left",0);
    ...
    public void UpdateArray2Mesh ()
	{
		...
		for (int i = 0; i < grid_array.pCol.Count; i++) { 
			for (int j = 0; j < grid_array.pCol [i].pRow.Count; j++) {				
				...
				uvs [tui] = grid_array.pCol [i].pRow [j].uv;
				tui++;
			}
		}
		...
		for (int i = 0; i < grid_array.pCol.Count - 1; i++) {
			...
			int next_i = i + 1;
			int c_count = grid_array.pCol [i].pRow.Count;
		
			for (int j = 0; j < c_count - 1; j++) {
				MP_MY_PTS p1, p2, p3, p4;
    ...
    /// </summary>
    /// <returns></returns>
    string getGammaKeyStr(int edge_id)
    {
        string Save_key = "S" + screen_id + "_" + edge_id + "_" + "Gamma";
        return Save_key;
    }
    /// <summary>
    ...
    public void Load()
    {
        ...
        for (int i = 0; i < edge_sides.Length; i++)
        {
            edge_sides[i].preloadGamma = PlayerPrefs.GetFloat(getGammaKeyStr(i), 0.333f);
        }
    }
    ...
    public void Save()
    {
        ...
        for (int i = 0; i < edge_sides.Length; i++)
        {
            if(edge_sides[i].mp_edge!=null)
                PlayerPrefs.SetFloat(getGammaKeyStr(i), edge_sides[i].mp_edge.gamma);
        }
  
    }

    /// <summary>
    /// update gamma to focused edge
    /// </summary>
    ...
    public void update_gamma(float gamma,bool isShowGamaLine)
    {
        ...
        //}        
        if(focusedEdge !=null)
            focusedEdge.mp_edge.UpdateGamma(gamma, true);
    }
    ...
    public float get_gamma()
    {
        ...
        if (focusedEdge != null)
            return focusedEdge.mp_edge.gamma;
        else
            return -1;
    }
    ...
    public void change_edge(int id, ref float edge_gamma)
	{
        ...
        if(edge_sides[id].isOpen) focusedEdge = edge_sides[id];
		UpdateEdge();
